...
barChart();
  ...
  function gaugeChart(){
    var inputData = d3.select("#selDataset").property("value");
    data.metadata.forEach(function(sample){
      if(sample.id == inputData){
        var frequency = sample.wfreq;
        var chart2 = [
          {
            domain: { x: [0, 1], y: [0, 1] },
            value: frequency,
        ...
        var chart2 = { width: 600, height: 600, margin: { t: 0, b: 0 } };
        Plotly.newPlot('gauge', selectData, chart2);
      }
    });
  }
  gaugeChart();
